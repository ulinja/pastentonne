# syntax=docker/dockerfile:1

FROM alpine:latest

ARG TIMEZONE
ENV TIMEZONE=${TIMEZONE:?error}
ARG UID
ENV UID=${UID:?error}
ARG GID
ENV GID=${GID:?error}
ARG NUXT_DATA_DIR
ENV NUXT_DATA_DIR=${NUXT_DATA_DIR:?error}
ARG NUXT_AUTH_ORIGIN
ENV NUXT_AUTH_ORIGIN=${NUXT_AUTH_ORIGIN:?error}
ARG NUXT_AUTH_SECRET
ENV NUXT_AUTH_SECRET=${NUXT_AUTH_SECRET:?error}
ARG NUXT_AUTHENTIK_CLIENT_ID
ENV NUXT_AUTHENTIK_CLIENT_ID=${NUXT_AUTHENTIK_CLIENT_ID:?error}
ARG NUXT_AUTHENTIK_CLIENT_SECRET
ENV NUXT_AUTHENTIK_CLIENT_SECRET=${NUXT_AUTHENTIK_CLIENT_SECRET:?error}
ARG NUXT_AUTHENTIK_ISSUER
ENV NUXT_AUTHENTIK_ISSUER=${NUXT_AUTHENTIK_ISSUER:?error}

RUN apk add --no-cache \
    nodejs \
    npm \
    tzdata

RUN cp "/usr/share/zoneinfo/${TIMEZONE}" /etc/localtime && \
    echo ${TIMEZONE:?error} > /etc/timezone

RUN (addgroup -g ${GID} pastentonne 2>/dev/null || true) && \
    adduser -u ${UID} -G "$(getent group ${GID} | cut -d: -f1)" -H -D pastentonne
RUN mkdir -p ${NUXT_DATA_DIR} && chown -R ${UID}:${GID} ${NUXT_DATA_DIR}
RUN mkdir /app && chown -R ${UID}:${GID} /app

COPY --chown=${UID}:${GID}             app /app/app
COPY --chown=${UID}:${GID} --chmod=744 docker/app.dev.entrypoint.sh /app/entrypoint.sh
COPY --chown=${UID}:${GID}             public /app/public
COPY --chown=${UID}:${GID}             components.json /app/components.json
COPY --chown=${UID}:${GID}             nuxt.config.ts /app/nuxt.config.ts
COPY --chown=${UID}:${GID}             package-lock.json /app/package-lock.json
COPY --chown=${UID}:${GID}             package.json /app/package.json
COPY --chown=${UID}:${GID}             tsconfig.json /app/tsconfig.json

USER ${UID}:${GID}
WORKDIR /app
ENV npm_config_cache=/tmp/.npm
RUN npm install
EXPOSE 8000
ENTRYPOINT ["/app/entrypoint.sh"]
